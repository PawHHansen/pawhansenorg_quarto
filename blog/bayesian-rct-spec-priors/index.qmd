...
```{r plottheme, echo = F, warning = F, message = F}
...
```{r sim-fake, warning=FALSE, message=FALSE}
...
```{r tbl-head-fake, warning=FALSE, message=FALSE, echo=FALSE}
fake |> 
  head(5) |> 
  kable(caption = "Peak at the first five rows in the data") |> 
...
```{r fig-weak-coef, fig.cap="Distribution of the two simulated parameters for our weakly-informative priors"}
...
```{r fig-weak-prior-hist, fig.cap="Checking the intercept. Simulated proportion of failed reading tests, given a weakly informative prior. The distribution is roughly uniform; reflecting that we have little idea about how many students in the control group would fail the reading test."}
...
```{r fig-evidence-coef, fig.cap="Distribution of the two simulated parameters for our evidence-based priors"}
...
```{r fig-prior-evidence, fig.cap="Checking the intercept. Simulated proportion of failed reading tests for the full sample, given an evidence-based prior. The proportion centers more narrowly around .5; reflecting that we have stronger beliefs."}
...
```{r fig-prior-treat, fig.cap="Checking the treatment effect. Simulated treatment effects from 100 simulated datasets, given an evidence-based prior. Most lines slope downward; reflecting our belief that fewer students in the treatment group will fail the test."}
set.seed(1407)
